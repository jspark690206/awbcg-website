...
export default function ContactForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMsg(e instanceof Error ? e.message : '오류가 발생했습니다. 잠시 후 다시 시도해 주세요.');
    }
  };
  ...
  if (status === 'success') {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
        ...
        <p className="text-gray-500 text-sm mb-6">담당자 확인 후 1~2 영업일 내 연락드리겠습니다.</p>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-5">
      ...
      <div>
        ...
        <textarea {...register('message')} rows={6} placeholder="도입을 검토 중인 시스템, 현재 운영 환경, 문의 사항 등을 자유롭게 작성해 주세요."
